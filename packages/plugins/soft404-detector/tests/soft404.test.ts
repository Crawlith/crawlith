...
describe('Soft 404 Detection Plugin', () => {
    ...
    const runPlugin = async (html: string, outLinks: number = 0) => {
        ...
        await Soft404DetectorPlugin.onMetricsPhase!(graph, { flags: { detectSoft404: true } } as any);
        return node as any;
    };
    ...
    it('should not do anything if flag is false', async () => {
        ...
        await Soft404DetectorPlugin.onMetricsPhase!(graph, { flags: { detectSoft404: false } } as any);

        expect(node.soft404Score).toBeUndefined();
    });
});
