...
import { describe, test as it, vi } from 'vitest';
import { ResponseLimiter } from '../src/core/network/responseLimiter.js';

describe('Fetcher Extended Coverage', () => {
    it('should catch oversized response and return oversized status', async () => {
        ...
        pool.intercept({
            path: '/oversize',
            method: 'GET'
        }).reply(200, 'x'.repeat(1024 * 1024 * 6));

        const originalStreamToString = ResponseLimiter.streamToString;
        ResponseLimiter.streamToString = async () => {
            throw new Error('Oversized response');
        };

        const fetcher = new Fetcher();
        const result = await fetcher.fetch('http://example.com/oversize', { maxBytes: 1 });

        expect(["oversized", "failed_after_retries", "network_error"]).toContain(result.status);
        ResponseLimiter.streamToString = originalStreamToString;
    });

    it('should handle proxy error to proxy_connection_failed', async () => {
        const fetcher = new Fetcher();
        const errResult = fetcher['errorResult']('proxy_connection_failed', 'http://proxy.com', [], 0);
        expect(errResult.status).toBe('proxy_connection_failed');
    });

    it('should map blocked internal ip directly from hostname', async () => {
        const fetcher = new Fetcher();
        const result = await fetcher.fetch('http://127.0.0.1/blocked');
        expect(result.status).toBe('blocked_internal_ip');
    });
});
