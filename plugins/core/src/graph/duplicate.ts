...
export function detectDuplicates(graph: Graph, options: DuplicateOptions = {}) {
    ...
    // Compile near duplicate clusters (group by root)
    const clusters = new Map<string, Set<GraphNode>>();

    for (const node of involvedNodes) {
        const root = uf.find(node.url);
        let set = clusters.get(root);
        if (!set) {
            set = new Set();
    ...
    for (const groupSet of clusters.values()) {
        if (groupSet.size > 1) {
