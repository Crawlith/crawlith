...
export const SimpleBarChart = () => {
  const { crawlDepthDistribution } = graphIntelligence;
  const maxCount = Math.max(...crawlDepthDistribution.map(d => d.count));
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm h-full flex flex-col">
      <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm mb-4">Crawl Depth Distribution</h3>

      <div className="flex-1 flex items-end justify-between gap-2 px-2 pb-2">
        {crawlDepthDistribution.map((item) => {
          const heightPercent = (item.count / maxCount) * 100;
          return (
            <div key={item.depth} className="flex flex-col items-center gap-2 flex-1 group">
              ...
              <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Depth {item.depth}</span>
            </div>
          );
        })}
      </div>
