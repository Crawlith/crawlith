...
test('crawl command execution (DB-only, no file writes)', async () => {
  ...
  await crawl.parseAsync(['https://example.com', '--limit', '10'], { from: 'user' });
...
test('crawl --export html flag triggers file export', async () => {
  ...
  await crawl.parseAsync(['https://example.com', '--limit', '10', '--export', 'html', '--output', 'test-output'], { from: 'user' });
...
test('crawl validates orphan severity flag dependency', async () => {
  ...
  await expect(
    crawl.parseAsync(['https://example.com', '--orphan-severity'], { from: 'user' })
...
test('crawl exits with code 1 when --fail-on-critical is set', async () => {
  ...
  await expect(
    crawl.parseAsync(['https://example.com', '--fail-on-critical'], { from: 'user' })
  ).rejects.toThrow('exit:1');
...
test('crawl diff execution via --compare', async () => {
  ...
  await crawl.parseAsync(['node', 'crawl', 'https://example.com', '--compare', 'old.json', 'new.json']);
